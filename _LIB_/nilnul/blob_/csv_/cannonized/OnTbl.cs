...
{
	static public class _OnTblX
	{

		public static DataTable ImportFromCsvFile(string filePath)
		{
			return ImportFromCsvFile(filePath, new[] { ',' });
		}
		/// <summary>
		/// 
		/// </summary>
		...
		/// </param>
		/// <returns></returns>
		public static DataTable ImportFromCsvFile(string filePath, char[] sep)
		{
			var rowIndex = 0;
			...
			try
			{
				using (var reader = new StreamReader(File.OpenRead(filePath)))
				{
					while (!reader.EndOfStream)
					{
						...
						if (nilnul.txt.be_.White.Singleton.be(line))
						{
							continue;
						}
		...
		public static DataTable ImportFromBlob(string blob)
		{
			return ImportFromBlob(blob, new char[] {','});
		}
		public static DataTable ImportFromBlob(string blob, char[] sep)
		{
			var lines = nilnul.txts.fro_.txt_.split_._LineX.Line_removeWhite(blob);
